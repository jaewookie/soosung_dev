...
LaserscanMerger::LaserscanMerger() : Node("laserscan_multi_merger")
{
	...
	point_cloud_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(cloud_destination_topic.c_str(), QOS_RKL10V);
	laser_scan_publisher_ = this->create_publisher<sensor_msgs::msg::LaserScan>(scan_destination_topic.c_str(), QOS_RKL10V);
}
	...
	if ((tmp_input_topics.size() != input_topics.size()) || !equal(tmp_input_topics.begin(), tmp_input_topics.end(), input_topics.begin()))
	{
		...
		if (input_topics.size() > 0)
		{
			...
			for (std::vector<int>::size_type i = 0; i < input_topics.size(); ++i)
			{
				...
				scan_subscribers[i] = this->create_subscription<sensor_msgs::msg::LaserScan>(input_topics[i].c_str(), rclcpp::QoS(1).best_effort(), callback);
